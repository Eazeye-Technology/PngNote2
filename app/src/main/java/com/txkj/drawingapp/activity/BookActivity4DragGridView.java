...
public class BookActivity4DragGridView extends GridView {
    //private boolean canDrag = false;
    public boolean getCanDrag() {
        if (getAdapter() instanceof BookPageGridAdapter) {
            return ((BookPageGridAdapter) getAdapter()).checkMode == BookPageGridAdapter.CHECK_MODE_MOVE;
        }
        return false;
    }
...
//    }
    ...
    private OnItemLongClickListener onLongClickListener = new OnItemLongClickListener(){
        ...
        public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            if (getAdapter() instanceof BookPageGridAdapter) {
                ((BookPageGridAdapter) getAdapter()).hideView(position);
            }
            return true;
        }
    };

    public BookActivity4DragGridView(Context context) {
        super(context);
        initView();
    }

    public BookActivity4DragGridView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public BookActivity4DragGridView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (getCanDrag()) {
            if (ev.getAction() == MotionEvent.ACTION_DOWN) {
                ...
                if ((currDraggedPosition != AdapterView.INVALID_POSITION) && (currDraggedPosition != preDraggedOverPositon)) {
                    ((BookPageGridAdapter) getAdapter()).swapView(preDraggedOverPositon, currDraggedPosition);
                    preDraggedOverPositon = currDraggedPosition;
                }
            }
            else if ((ev.getAction() == MotionEvent.ACTION_UP) && (isViewOnDrag == true)) {
                ((BookPageGridAdapter) getAdapter()).showHideView();
                if ((int) dragImageView.getTag() == DRAG_IMG_SHOW) {
                    windowManager.removeView(dragImageView);
                    dragImageView.setTag(DRAG_IMG_NOT_SHOW);

                    if (this._runnableDrop != null) {
                        this._runnableDrop.run();
                    }
                }
                isViewOnDrag = false;
            }
    ...
    private Runnable _runnableDrop = null;
    public void setOnDrop(Runnable runnableDrop) {
        this._runnableDrop = runnableDrop;
    }
}