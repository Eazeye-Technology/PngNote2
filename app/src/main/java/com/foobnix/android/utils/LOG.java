...
public class LOG {
    ...
    private static void e(Throwable e, Boolean uncaughtException, Object... statement) {
        ...
        if (writeCrashTofile) {
            try {
                String rootPath = new File(Environment.getExternalStorageDirectory(), NoteFragment2.APPNAME_NEW/*"txkjreader"*/ /*"Librera"*/).toString();
                File SYNC_FOLDER_ROOT = new File(rootPath);
                FileWriter fw = new FileWriter(new File(SYNC_FOLDER_ROOT, "crash.txt"), true);
